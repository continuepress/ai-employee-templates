# NOTE: This is the same job contract as CLAUDE.md, named for agents that
# auto-read AGENTS.md (Codex CLI and others following the AGENTS.md convention).
# Keep whichever file your agent loads automatically; delete the other,
# or keep both in sync if you switch tools.
...
- **Checkpoint every ~1 hour of work or after each completed task**: update `STATE.md`; if this folder is a git repository, also `git add -A && git commit`. A session can die at any moment, so anything not in files is LOST.
